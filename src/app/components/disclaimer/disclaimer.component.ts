...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-4 transition-colors duration-300">
      <div class="max-w-2xl w-full bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-10 border border-slate-200 dark:border-white/5">
        ...
        <label class="flex items-center space-x-4 cursor-pointer group mb-10 p-4 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700/30 transition-colors">
          <div class="relative flex items-center justify-center">
            ...
            <div class="w-6 h-6 border-2 border-slate-300 dark:border-slate-500 rounded bg-white dark:bg-slate-800 peer-checked:bg-blue-500 peer-checked:border-blue-500 transition-all peer-focus:ring-2 peer-focus:ring-blue-500/50"></div>
        ...
        <div class="flex space-x-4">
          <button (click)="goBack()" class="flex-1 py-4 px-6 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-700 dark:text-slate-300 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 focus:bg-slate-200 dark:focus:bg-slate-600 hover:text-slate-900 dark:hover:text-white transition-colors focus:ring-2 focus:ring-slate-500 focus:outline-none">
            Back
          </button>
          
          <button (click)="start()" 
                  [disabled]="!acknowledged()"
                  class="flex-1 flex justify-center py-4 px-6 border border-transparent rounded-xl text-white font-bold bg-blue-600 hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:ring-offset-white dark:focus:ring-offset-slate-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-blue-600 shadow-lg shadow-blue-500/25">
            Start Exam
          </button>
        </div>
